#!/usr/bin/env python3
"""Extract C2 traffic from selected few PCAP files and add to existing C2 data.

This script processes only 2-3 selected PCAP files to add more C2 traffic.
"""
...
def add_c2_to_existing():
    """Extract C2 from 2-3 selected PCAPs and add to existing dataset."""
    
    # Configuration
    selected_pcaps = [
        "C:/Users/akshi/Downloads/botnet-capture-20110811-neris (1).pcap",
        "C:/Users/akshi/Downloads/botnet-capture-20110816-qvod.pcap",
        "C:/Users/akshi/Downloads/botnet-capture-20110815-fast-flux.pcap"
    ]
    
    existing_c2_path = Path("data/processed/balanced_ddos_c2.csv")
    output_path = Path("data/processed/enhanced_ddos_c2.csv")
    
    logger.info("🚀 Starting selective C2 extraction...")
    ...
    try:
        existing_df = pd.read_csv(existing_c2_path)
        logger.info(f"✅ Loaded existing dataset: {len(existing_df)} samples")
        
        # Separate existing C2 and DDoS
        existing_c2 = existing_df[existing_df['Attack_Type'] == 'C2'].copy()
        existing_ddos = existing_df[existing_df['Attack_Type'] == 'DDoS'].copy()
        
        logger.info(f"📊 Existing C2: {len(existing_c2)}, DDoS: {len(existing_ddos)}")
        
    except FileNotFoundError:
        logger.error(f"❌ Existing dataset not found: {existing_c2_path}")
        return
    
    # Extract C2 from selected PCAPs
    new_c2_data = []
    ...
    for pcap_path in selected_pcaps:
        ...
        if not pcap_file.exists():
            logger.warning(f"⚠️ PCAP not found: {pcap_path}")
            continue
        ...
        if flows_df is not None:
            ...
            if c2_data is not None and not c2_data.empty:
                ...
                if c2_splt is not None:
                    new_c2_data.append(c2_splt)
    
    if not new_c2_data:
        logger.error("❌ No new C2 traffic found")
        return
    
    # Combine new C2 data
    new_c2_combined = pd.concat(new_c2_data, ignore_index=True)
    logger.info(f"📊 New C2 flows extracted: {len(new_c2_combined)}")
    
    # Combine with existing data
    all_c2 = pd.concat([existing_c2, new_c2_combined], ignore_index=True)
    
    # Create balanced dataset
    c2_count = len(all_c2)
    ddos_count = len(existing_ddos)
    
    if c2_count > ddos_count:
        # Sample C2 to match DDoS
        c2_sample = all_c2.sample(n=ddos_count, random_state=42)
        ddos_sample = existing_ddos
        logger.info(f"📊 Sampled {ddos_count} C2 flows from {c2_count}")
    else:
        # Use all C2 and sample DDoS to match
        c2_sample = all_c2
        ddos_sample = existing_ddos.sample(n=c2_count, random_state=42)
        logger.info(f"📊 Sampled {c2_count} DDoS flows from {ddos_count}")
    
    # Combine final dataset
    final_df = pd.concat([ddos_sample, c2_sample], ignore_index=True)
    ...
    logger.info(f"📁 Enhanced dataset: {output_path} ({output_path.stat().st_size / (1024*1024):.2f} MB)")
