...
# Categorize features
splt_features = [col for col in df.columns if 'splt_len_' in col]
iat_features = [col for col in df.columns if 'splt_iat_' in col]
basic_flow_features = ['duration', 'protocol', 'total_packets', 'total_bytes', 'fwd_packets', 'bwd_packets', 'fwd_bytes', 'bwd_bytes']
...
metadata_features = ['flow_id', 'src_ip', 'src_port', 'dst_ip', 'dst_port', 'attack_type', 'label', 'source_file']

print(f'\n📊 FEATURE BREAKDOWN:')
print(f'SPLT Length features (packet sizes): {len(splt_features)}')
print(f'SPLT IAT features (inter-arrival times): {len(iat_features)}')
...
for i, feat in enumerate(splt_features[:10]):
    print(f'{i+1:2d}. {feat}')

print(f'\n⏱️ SPLT IAT FEATURES (First 10):')
for i, feat in enumerate(iat_features[:10]):
    print(f'{i+1:2d}. {feat}')
